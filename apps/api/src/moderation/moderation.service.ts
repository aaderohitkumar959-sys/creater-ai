...
export class ModerationService {
   ...
   * Used by chat service
   */
  async logViolation(userId: string, reason: string, content: string): Promise<void> {
    try {
  ...
  async moderateContent(text: string): Promise<ModerationResult> {
    ...
    try {
      ...
      if (result.categories['sexual/minors']) {
        ...
        return { blocked: true, reason: 'ILLEGAL_CONTENT', categories: ['sexual/minors'], severity: 'CRITICAL' };
      }

      // HIGH: Block hate speech and violence
      if (result.categories.hate || result.categories['hate/threatening']) {
        return { blocked: true, reason: 'HATE_SPEECH', categories: ['hate'], severity: 'HIGH' };
      }

      if (result.categories.violence || result.categories['violence/graphic']) {
        return { blocked: true, reason: 'VIOLENCE', categories: ['violence'], severity: 'HIGH' };
      }

      // MEDIUM: Block sexual content (not involving minors)
      if (result.categories.sexual) {
        return { blocked: true, reason: 'SEXUAL_CONTENT', categories: ['sexual'], severity: 'MEDIUM' };
      }

      // LOW: Self-harm content (soft block with resources)
      if (result.categories['self-harm']) {
        return { blocked: true, reason: 'SELF_HARM', categories: ['self-harm'], severity: 'LOW' };
      }
  ...
  private async reportToNCMEC(content: string) {
    ...
    console.error('[NCMEC] REPORT REQUIRED (Mock):', { timestamp: new Date().toISOString() });
  ...
  getUserMessage(result: ModerationResult): string {
    switch (result.reason) {
      case 'ILLEGAL_CONTENT': return 'This content violates our terms and has been reported.';
      case 'HATE_SPEECH': return 'This message contains hate speech.';
      case 'VIOLENCE': return 'This message contains violent content.';
      case 'SEXUAL_CONTENT': return 'This message contains inappropriate sexual content.';
      case 'SELF_HARM': return 'We detected content related to self-harm. Please reach out to 988.';
      default: return 'This message violates our community guidelines.';
    }
  }
}
